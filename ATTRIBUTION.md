...
| Column-building packing — the vertical transpose of block-building: grow full-height columns of a fixed XY footprint (largest profile first), where each column slice is a footprint-sized mini-layer fused from items (reusing the block packer's footprint grouping + 2-D MaxRects tiling), tile the columns across the floor, then EMS void-fill the leftover space. The vertical-slice / wall framing is the wall-building tradition (George & Robinson, 1980); the per-column footprint fusion reuses our block-packer machinery. The column-of-largest-profile orchestration + EMS top-fill is our own composition. | [`d3/columns.go`](d3/columns.go) |
...
| Access-cost objective: minimise Σ (how often an item is wanted × what its position costs to reach), with the cost a weighted sum of height, walking distance, fixed container-opening cost and per-scalar terms including a height×scalar interaction | Storage-assignment / class-based slotting in warehousing (cube-per-order index, Heskett 1963); no single packing source | [`d3/access.go`](d3/access.go), [`joint/jointfit.go`](joint/jointfit.go) |
| Exclusion (keep-out) zones: axis-aligned regions no item may occupy, granting no support and consuming no usable volume | Standard container-loading practice (wheel arches, fixtures); no single source | [`d3/zones.go`](d3/zones.go) |
